 ...
 */
abstract class AbstractConcurrentStreamBuilder<E, A, S> {
     ...
     * is {@code 2^(k + 1 - i)} where {@code k} is this value and {@code i}
     * is the initial {@link Node} capacity.
     */
    ...
    abstract static class Node<A, S> {
         ...
         */
        Node(final int capacity, final int expectedCount) {
            this(capacity);
    ...
    AbstractConcurrentStreamBuilder() {
        this(DEFAULT_INITIAL_CAPACITY);
    }

    AbstractConcurrentStreamBuilder(final int initialCapacity) {
    ...
    protected abstract class AbstractSpliterator {
